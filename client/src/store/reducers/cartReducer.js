export const cartReducer = (initialState = { cart: [] }, action) => {
	switch (action.type) {
		case "ADD_TO_CART": {
			const { id, name, img, price, quantity, size } = action.payload;
			if (!initialState.cartItems.find((x) => x.id === id)) {
				initialState.cartItems.push({ id, name, img, price, quantity, size });
				localStorage.setItem("cart", JSON.stringify(initialState.cartItems));
				return { ...initialState };
			} else if (
		...
		case "REMOVE_FROM_CART": {
			...
			if (initialState.cartItems.find((x) => x.id === id)) {
				const newCart = initialState.cartItems.filter((x) => x.id !== id);
		...
		case "ADD_ADDRESS": {
			const { address, district, postalCode, country, state } = action.payload;
